<section class="my-5 my-lg-10 bg-ticket">
    <div class="py-5 py-lg-10">
        <div class="container">
            <div class="row">
                <div class="col-lg-12">
                    <div class="text-center mb-12">
                        <div class="fs-4x text-base font-title ls-4 mb-7" id="tiket"
                            data-kt-scroll-offset="{default: 100, lg: 150}">
                            TIKET
                        </div>
                    </div>
                </div>
            </div>
            <div class="row justify-content-center px-8 px-lg-10">
                <div class="col-xl-9 col-xxl-10">
                    <div class="row">
                        <div class="col-lg-4 col-sm-6">
                            <a href="#" class="card bg-transparent">
                                <div class="bgi-no-repeat bgi-size-contain bgi-position-center position-relative min-h-175px mw-100 h-auto"
                                    style="background-image: url('<?php echo e(asset('/assets/media/ticket.png')); ?>')">
                                    <div class="card-body">
                                        <div class="d-flex justify-content-start flex-column flex-wrap mb-5">
                                            <div class="fs-2x text-base font-title">
                                                Early Bird
                                            </div>
                                            <div class="fs-4 fst-italic text-base  fw-semibold">
                                                2D Pass
                                            </div>
                                        </div>
                                        <div class="fs-2hx text-base fw-bold">
                                            Rp 200.000
                                        </div>
                                    </div>
                                </div>
                            </a>
                        </div>
                        <div class="col-lg-4 col-sm-6">
                            <a href="#" class="card bg-transparent">
                                <div class="bgi-no-repeat bgi-size-contain bgi-position-center position-relative min-h-175px mw-100 h-auto"
                                    style="background-image: url('<?php echo e(asset('/assets/media/ticket.png')); ?>')">
                                    <div class="card-body">
                                        <div class="d-flex justify-content-start flex-column flex-wrap mb-5">
                                            <div class="fs-2x text-base font-title">
                                                Presale
                                            </div>
                                            <div class="fs-4 fst-italic text-base  fw-semibold">
                                                2D Pass
                                            </div>
                                        </div>
                                        <div class="fs-2hx text-base fw-bold">
                                            Rp 250.000
                                        </div>
                                    </div>
                                </div>
                            </a>
                        </div>
                        <div class="col-lg-4 col-sm-6">
                            <a href="#" class="card bg-transparent">
                                <div class="bgi-no-repeat bgi-size-contain bgi-position-center position-relative min-h-175px mw-100 h-auto"
                                    style="background-image: url('<?php echo e(asset('/assets/media/ticket.png')); ?>')">
                                    <div class="card-body">
                                        <div class="d-flex justify-content-start flex-column flex-wrap mb-5">
                                            <div class="fs-2x text-base font-title">
                                                Normal
                                            </div>
                                            <div class="fs-4 fst-italic text-base  fw-semibold">
                                                2D Pass
                                            </div>
                                        </div>
                                        <div class="fs-2hx text-base fw-bold">
                                            Rp 350.000
                                        </div>
                                    </div>
                                </div>
                            </a>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>
<?php /**PATH /Users/yeremiasatrio/Project/wonogiri-fest/resources/views/frontend/home/ticket.blade.php ENDPATH**/ ?>